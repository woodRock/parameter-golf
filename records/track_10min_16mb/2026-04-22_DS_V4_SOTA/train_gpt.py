...
def flex_attention(q, k, v, cu_seqlens=None, max_seqlen=0, causal=True):
    ...
    if cu_seqlens is not None:
        ...
        # q layout is (B, T, Hq, d); k/v are (B, T, Hkv, d) with Hkv <= Hq (GQA).
        # Reshape to (n_docs, max_seqlen, H, d) per-tensor and batch over docs.
        B, T, Hq, d = q.shape[0], q.shape[1], q.shape[2], q.shape[3]
        Hkv = k.shape[2]
        n_docs = T // max_seqlen
        q_d = q.reshape(n_docs, max_seqlen, Hq, d)
        k_d = k.reshape(n_docs, max_seqlen, Hkv, d)
        v_d = v.reshape(n_docs, max_seqlen, Hkv, d)
...
def _compressed_code_size(code):
    ...
    try:
        ...
        # Try python -m python_minifier
        try:
            minified = subprocess.run(
                [sys.executable, "-m", "python_minifier", "--no-rename-locals", "--no-hoist-literals", "--remove-literal-statements", "-"],
                input=code_raw, capture_output=True, check=True,
            ).stdout
        except (subprocess.CalledProcessError, FileNotFoundError):
            pass
